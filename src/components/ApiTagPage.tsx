...
function schemaToExample(
    ...
    if (schema.type === 'array' && schema.items)
        return [schemaToExample(schema.items, spec, depth + 1)]
...
function buildCurl(
    ...
    if (required.length)
        url += '?' + required.map((p) => `${p.name}=VALUE`).join('&')
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-border text-left text-muted-foreground">
                                    <th className="pb-2 pl-2 pr-6 font-medium">
                                        Name
                                    </th>
                                    <th className="pb-2 pl-2 pr-6 font-medium whitespace-nowrap">
                                        In
                                    </th>
                                    <th className="pb-2 pl-2 pr-6 font-medium whitespace-nowrap">
                                        Type
                            ...
                            <tbody>
                                {params.map((p, i) => {
                                    ...
                                    return (
                                        <tr
                                            key={i}
                                            className="border-b border-border/40 odd:bg-muted last:border-0"
                                        >
                                            <td className="p-2 pr-6 align-top font-mono">
                                                {p.name}
                                                {p.required && (
                                            ...
                                            <td className="p-2 pr-6 align-top text-muted-foreground whitespace-nowrap">
                                                {p.in}
                                            </td>
                                            <td className="p-2 pr-6 align-top font-mono text-muted-foreground whitespace-nowrap">
                                                {s?.type ?? '—'}
                                            </td>
...
const ApiTagPage: React.FC<{ tag: string }> = ({ tag }) => {
    ...
    useEffect(() => {
        ...
        injectToc(ops)
        return () => {
            document.getElementById('api-toc-list')?.remove()
        }
    }, [spec, tag])
