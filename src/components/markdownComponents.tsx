...
export const markdownComponents: Components = {
    ...
    hr: () => <hr className="my-4 border-t border-(--grid-line-color)" />,
    a: ({ children, href }) => (
        <a
            href={href}
            target="_blank"
    ...
    th: ({ children }) => (
        <th className="border border-(--grid-line-color) bg-muted/40 px-3 py-2 text-left font-semibold">
            {children}
        </th>
    ),
    td: ({ children }) => (
        <td className="border border-(--grid-line-color) px-3 py-2">
            {children}
        </td>
    ),
}
