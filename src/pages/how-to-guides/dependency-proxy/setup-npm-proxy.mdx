---
title: Setup npm Proxy with DevGuard Dependency Proxy
description: "Configure npm to route package downloads through the DevGuard dependency proxy for malicious package detection and supply chain security."
seo:
  robots: index,follow
  og:
    image: /og-image.png
    type: article
  schema:
    type: TechArticle
  keyword_primary: setup npm proxy with devguard
lang: en-US
ignoreChecks: null
---

import { Callout } from '@document-writing-tools/kernux-theme'

# Setup npm Proxy with DevGuard Dependency Proxy

Supply chain attacks through npm are a growing threat. In 2025, attackers published malicious packages impersonating the popular TanStack library, targeting developers who mistyped the package name or were misled by search results. Incidents like this — where a single `npm install` can silently compromise a developer machine or CI environment — highlight why every package download should be screened before it executes on your systems.

The DevGuard dependency proxy sits between your developers and the public npm registry. Every package request is checked against the [OSV dataset](https://osv.dev) before it reaches your machine, blocking known malicious packages automatically.

- **Registry URL**: `<your-devguard-url>/api/v1/dependency-proxy/npm`

## Configuration

Point npm at the DevGuard proxy by adding a registry entry to your `.npmrc`. This file can live at the project level (`./.npmrc`) to scope only that project, or at the user level (`~/.npmrc`) to apply globally.

```ini
registry=https://<your-devguard-url>/api/v1/dependency-proxy/npm
```

Once set, all `npm install` and `npm ci` invocations route through DevGuard transparently. No changes to your `package.json` or CI scripts are required.

<Callout type="warning">
  Set `strict-ssl=false` only for local testing against a self-signed certificate. Always use `true` in production.
</Callout>

<Callout type="info">
  On startup, the proxy blocks all requests until the malicious package database is fully loaded. This initialization period prevents cache poisoning. See [Cache Management](/how-to-guides/dependency-proxy/cache-management) for details.
</Callout>

## Testing

DevGuard ships a test package that is permanently flagged as malicious. Use it to verify the proxy is working before relying on it in production:

```json
{
  "dependencies": {
    "lodash": "^4.17.21",
    "fake-malicious-npm-package": "1.0.0"
  }
}
```

Run `npm install`. `lodash` installs successfully while `fake-malicious-npm-package` is rejected with a `403 Forbidden`. If you see both results, your proxy is configured correctly.

`npm audit` and `npm audit signatures` keep working behind the proxy: DevGuard forwards the audit requests, the registry signing keys and the provenance attestations to the npm registry.

<Callout type="info">
  `npm audit signatures` prints `Fetching verification keys using TUF failed. Fetching directly from <registry>.` when run against the proxy. This is expected: npm first looks up the signing keys of the configured registry in the [Sigstore TUF repository](https://github.com/sigstore/root-signing), which only contains the keys of public registries such as `registry.npmjs.org`. Therefore tuf cant be used to fetch the signatures. npm itself will fallback to fetch the keys from the proxy, which forwards the official keys of the npm registry, and verifies the signatures with them.
</Callout>

The same check as a script, installing each package separately so the failure of the malicious one is visible:

```bash
mkdir -p npm-proxy-test && cd npm-proxy-test
npm init -y > /dev/null
echo "registry=https://<your-devguard-url>/api/v1/dependency-proxy/npm" > .npmrc

# lodash installs through the proxy
npm install lodash@^4.17.21

# the audit is forwarded to the npm registry
# (--audit-level=none only fails if the audit itself fails, not on reported vulnerabilities)
npm audit --audit-level=none

# registry signatures of the installed packages are verified through the proxy
npm audit signatures

# fake-malicious-npm-package must be rejected with 403 Forbidden
if npm install fake-malicious-npm-package@1.0.0; then
  echo "fake-malicious-npm-package was NOT blocked - check your proxy configuration" >&2
  exit 1
fi
```

### Testing the minimum package age

The [minimum package age](/how-to-guides/dependency-proxy#minimum-package-age) is configured per repository and only applies to requests using the repository's proxy URL, which contains a secret. To verify it, temporarily set the minimum age to 87600 hours (10 years). `lodash@4.17.21` was published in February 2021, so the proxy must reject it, while a version range still resolves to an older version:

<Callout type="info">
  npm 11 has its own client-side minimum release age (`min-release-age` in `.npmrc`, in days). If it is set, npm hides recent versions itself and reports errors like `No matching version found for lodash@4.17.21 with a date before …` — independent of the proxy. The script passes `--min-release-age=0` so that only the proxy decides which versions are available. Both settings can be combined in practice; the proxy enforces the minimum age centrally, **even for clients that do not support or configure it**.
</Callout>

```bash
mkdir -p npm-min-age-test && cd npm-min-age-test
npm init -y > /dev/null

# set the minimum package age of the repository to 10 years
devguard-scanner curl --token <your-pat-token> -X PUT \
  -d '{"rules":"","minReleaseAge":87600}' \
  https://<your-devguard-url>/api/v1/organizations/<assetName>/config-files/dependency-proxy-configs/

# the repository's npm proxy URL applies its settings
NPM_PROXY=$(devguard-scanner curl --token <your-pat-token> -s \
  https://<your-devguard-url>/api/v1/organizations/<assetName>/dependency-proxy-urls/ \
  | node -pe 'JSON.parse(require("fs").readFileSync(0, "utf8")).npm')

# --min-release-age=0 disables npm's own minimum release age (npm 11+),
# so only the proxy decides which versions are available

# lodash@4.17.21 is younger than 10 years and must be rejected
if npm install lodash@4.17.21 --prefer-online --min-release-age=0 --registry "$NPM_PROXY"; then
  echo "lodash@4.17.21 was NOT blocked by the minimum package age" >&2
  exit 1
fi

# a version range resolves to the newest version that is old enough - this will install a really outdated version like 4.16.2
npm install "lodash@^4" --prefer-online --min-release-age=0 --registry "$NPM_PROXY"

# reset the minimum package age
devguard-scanner curl --token <your-pat-token> -X PUT \
  -d '{"rules":"","minReleaseAge":0}' \
  https://<your-devguard-url>/api/v1/organizations/<assetName>/config-files/dependency-proxy-configs/
```

## Further Reading

- [Dependency Proxy Overview](/how-to-guides/dependency-proxy)
- [Cache Management](/how-to-guides/dependency-proxy/cache-management)
- [Setup Go Proxy](/how-to-guides/dependency-proxy/setup-go-proxy)
- [Setup PyPI Proxy](/how-to-guides/dependency-proxy/setup-pypi-proxy)
- [OSV (Open Source Vulnerabilities)](https://osv.dev)
