...
## How to Ingest Upstream Information 📩
...
**Manual File Upload:**
1. Select **"Costum Setup"** →  **"Use your own scanner"** → **"Upload manually"**
...
When you delete an information source that references a vulnerability, DevGuard removes that vulnerability from the affected component—**unless** another
...
- **Multiple sources:** If you also uploaded an SBOM (which finds this vulnerability) from "Scanner B" reporting the same component (and therefore vulnerability), deleting the VEX
  document will not remove the vulnerability — it remains associated with the component via Scanner B
...
- `exploitability-statement` — References to external vulnerability analysis
- `bom` — References to external SBOM documents
...
1. **State** — The vulnerability status (e.g., `not_affected`, `affected`, `fixed`)
2. **Justification** — The reason for the state (when provided)
...
**Available Endpoints:**
- `/api/v1/public/{assetId}/vex.json` — Public VEX document with vulnerability assessments
- `/api/v1/public/{assetId}/sbom.json` — Public SBOM with component inventory
...
- `exploitability-statement` — Links to vulnerability analysis documents
- `bom` — Links to upstream SBOM documents
