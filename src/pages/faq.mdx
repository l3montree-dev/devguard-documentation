...
Make sure the Personal Access Token (PAT) you configured for the scanner is still valid and has not expired or been revoked. You can manage tokens in DevGuard under **Settings → Personal Access Tokens**.
...
If the token is invalid, create a new one with the `scan` scope and update it in your CI/CD configuration.
</Steps>
...
Once enabled, anyone with the link can view the security dashboard without being logged in.