...
Free and Open Source Software (FOSS). This heavy reliance on external dependencies has fundamentally altered the [threat landscape](/explanation/Supply-Chain-Security/what-is-supply-chain-security#the-threat-landscape),
...
[**Source**](/explanation/Supply-Chain-Security/what-is-supply-chain-security#1-source-threats),
[**Build**](/explanation/Supply-Chain-Security/what-is-supply-chain-security#2-build-threats),
[**Dependency**](/explanation/Supply-Chain-Security/what-is-supply-chain-security#3-dependency-threats),
and [**Deployment & Runtime threats**](/explanation/Supply-Chain-Security/what-is-supply-chain-security#4-deployment-and-runtime-threats).

![Software Supply Chain Threat Model](/threat-landscape.png)

### 1. Source Threats

These risks target the earliest stage of development—the code itself and the systems that manage it. Attackers aim to compromise source code repositories to inject malicious code or steal intellectual property
...
**Impact:** The attack was discovered by chance just weeks before it would have merged into stable Linux distributions. While this miracle discovery prevented the infection of **hundreds of millions of servers** globally,
...
This incident illustrates a critical failure in build security. Attackers compromised the **build environment and continuous integration server**, allowing them to modify and infect software updates
...
The **Log4Shell** vulnerability in the ubiquitous Java logging library **Log4j** demonstrates the massive reach of dependency threats. A critical flaw in how the library processed log messages allowed attackers to
...
highlighting how a single runtime vulnerability can lead to catastrophic data loss.
...
While signing proves _who_ signed an artifact, it does not prove _how_ it was built. A signed binary could still be malicious if it was built on a compromised laptop or build server.
...
The [in-toto](tutorials/advanced/supply-chain-security) specification is the industry standard for this attestation data.
...
To standardize the security pillars described above, the industry relies on two primary frameworks:\
[**NIST SSDF**](https://csrc.nist.gov/projects/ssdf) and [**SLSA**](https://slsa.dev/).
While they share the same goal, they approach the problem from different perspectives: **SSDF** focuses on the **process**, while **SLSA** focuses on the **artifact**.
...
[**Supply-chain Levels for Software Artifacts (SLSA)**](/explanation/supply-chain-security/slsa-framework.mdx) is a framework specifically designed to guarantee the integrity of the final software output.
...
SLSA relies on the [in-toto framework](explanation/supply-chain-security/in-toto-framework) to provide the standard format for this metadata.
...
**Shift Left** moves these security processes to the **earlies** possible point in the development timeline:
...
Supply Chain Security is no longer optional in an era of automated, multi-layered software delivery. It requires moving beyond simple vulnerability scanning and into the realm of **provenance and integrity**.
By understanding the flow of code from source to production, and by demanding cryptographic proof of every transformation, organizations can significantly reduce the risk of sophisticated supply chain attacks.

In the following sections, we will explore the specific frameworks **DevGuard** uses to implement these concepts, including [In-toto](/explanation/in-toto-framework.mdx) and [SLSA](/explanation/slsa-framework.mdx).
