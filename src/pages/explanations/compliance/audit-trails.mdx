...
<Callout type="info" emoji="📋">
    Audit logging answers: "How did you handle this vulnerability? Who approved
    risk acceptance? When was it fixed?" Without trails, these become manual
    reconstruction exercises consuming audit time.
</Callout>
...
**Initial Detection**: When scanners identify a <Tooltip><TooltipTrigger asChild><span className="custom:underline custom:decoration-dotted custom:decoration-yellow-400 custom:decoration-1 custom:underline-offset-4 custom:cursor-pointer">CVE</span></TooltipTrigger><TooltipContent><p>Common Vulnerabilities and Exposures - Standardized vulnerability identifier</p></TooltipContent></Tooltip>, DevGuard logs discovery timestamp, affected component, initial risk score, and detection method.
...
DevGuard recalculates risk scores multiple times daily. Every change is logged with reasoning: new <Tooltip><TooltipTrigger asChild><span className="custom:underline custom:decoration-dotted custom:decoration-yellow-400 custom:decoration-1 custom:underline-offset-4 custom:cursor-pointer">EPSS</span></TooltipTrigger><TooltipContent><p>Exploit Prediction Scoring System - Exploitation probability</p></TooltipContent></Tooltip> data, exploit publication, patch availability, component depth changes.
...
<Callout type="info" emoji="🔄">
    Component removal provides automatic remediation evidence—delete a
    vulnerable library, audit logs prove the vulnerability no longer affects
    your application without manual documentation.
</Callout>
...
Export formats: PDF (human review), JSON/CSV (automated tools), <Tooltip><TooltipTrigger asChild><span className="custom:underline custom:decoration-dotted custom:decoration-yellow-400 custom:decoration-1 custom:underline-offset-4 custom:cursor-pointer">VEX</span></TooltipTrigger><TooltipContent><p>Vulnerability Exploitability eXchange - Machine-readable assessments</p></TooltipContent></Tooltip> (supply chain partners).

***

## Related Documentation

- [Vulnerability Event System](/explanations/vulnerability-management/vulnerability-events) - Actions generating audit events
- [Vulnerability Lifecycle](/explanations/vulnerability-management/vulnerability-lifecycle) - Complete management process
