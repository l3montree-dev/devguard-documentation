---
title: Setup a Fresh DevGuard Instance and Scan for Vulnerabilities
description: "Get started with the DevGuard quick start guide and run your first security scan in minutes — open-source platform for vulnerability and SBOM management."
seo:
  robots: index,follow
  og:
    image: /og-image.png
    type: article
  keyword_primary: setup a fresh devguard instance
  schema:
    type: TechArticle
lang: en-US
ignoreChecks: null
...
First, clone the [devguard-docker-deployment](https://github.com/l3montree-dev/devguard-docker-deployment) repo, run the guided setup, and start DevGuard:
...
The guided setup writes `.env` and generates the encryption key, the Kratos config, and the database init script, then the services are started in the background.
...
```bash
docker pull ghcr.io/l3montree-dev/devguard/scanner:main
```
