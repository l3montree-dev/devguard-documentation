...
2. **Create necessary existing secrets**
    - DevGuard requires it's own private key to sign In-Toto Root-Layouts (Refer to the [In-Toto Whitepaper](/concepts/attestations-provenance/in-toto)).
       ```bash
...
For the [CSAF Support in DevGuard](/feature-guides/csaf-common-security-advisory-framework) it requires a secret containing your PGP key pair.
```bash
...
Then set the secret name via `api.existingCsafSecretName` in your `values.yaml` file.
